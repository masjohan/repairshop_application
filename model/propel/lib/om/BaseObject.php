...
abstract class BaseObject
{
  ...
  /* TAO add */
  public function getCreated($action = 'INSERT') {
    $id = $this->getId();
    $cls = get_class($this);
    $peerCls = $cls::PEER;
    $tb = $peerCls::TABLE_NAME;

    $con = Propel::getConnection($peerCls::DATABASE_NAME, Propel::CONNECTION_READ);
    $stmt = $con->prepare('SELECT `when` FROM _audit WHERE table_name=? AND table_id=? AND action=? ORDER BY `when` DESC LIMIT 1');
    $stmt->execute(array($tb, $id, $action));
    list(list($when)) = $stmt->fetchAll(PDO::FETCH_NUM);

    return $when;
  }
  public function getUpdated() {
    return $this->getCreated('UPDATE');
  }
}
