...
/**
 * Base class that represents a row from the 'RepairOrderStatus' table.
 *
 * 
 *
 * @package    propel.generator.repairshop.om
 */
abstract class BaseRepairorderstatus extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'RepairorderstatusPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        RepairorderstatusPeer
	 */
	 ...
	 * @return     Repairorderstatus The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = RepairorderstatusPeer::ID;
		}
	...
	public function setStatus($v)
	{
		...
		if ($this->status !== $v) {
			$this->status = $v;
			$this->modifiedColumns[] = RepairorderstatusPeer::STATUS;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = RepairorderstatusPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Repairorderstatus object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RepairorderstatusPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				RepairorderstatusPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(RepairorderstatusPeer::ID) ) {
					...
					$affectedRows = RepairorderstatusPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RepairorderstatusPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RepairorderstatusPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Repairorderstatus'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Repairorderstatus'][$this->getPrimaryKey()] = true;
		$keys = RepairorderstatusPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RepairorderstatusPeer::ID)) $criteria->add(RepairorderstatusPeer::ID, $this->id);
		if ($this->isColumnModified(RepairorderstatusPeer::STATUS)) $criteria->add(RepairorderstatusPeer::STATUS, $this->status);
	...
	{
		$criteria = new Criteria(RepairorderstatusPeer::DATABASE_NAME);
		$criteria->add(RepairorderstatusPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Repairorderstatus (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Repairorderstatus Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     RepairorderstatusPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new RepairorderstatusPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this Repairorderstatus is new, it will return
		...
		if(null === $this->collRepairorders || null !== $criteria) {
			if ($this->isNew() && null === $this->collRepairorders) {
				...
				$collRepairorders = RepairorderQuery::create(null, $criteria)
					->filterByRepairorderstatus($this)
					->find($con);
				if (null !== $criteria) {
	 ...
	 * Otherwise if this Repairorderstatus is new, it will return
	 * an empty collection; or if this Repairorderstatus has previously
	 * been saved, it will retrieve related Repairorders from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Repairorderstatus.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 ...
	 * @return     PropelCollection|array Repairorder[] List of Repairorder objects
	 */
	public function getRepairordersJoinVehicle($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = RepairorderQuery::create(null, $criteria);
	...
	{
		return (string) $this->exportTo(RepairorderstatusPeer::DEFAULT_STRING_FORMAT);
	}
