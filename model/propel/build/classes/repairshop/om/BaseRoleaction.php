...
/**
 * Base class that represents a row from the 'RoleAction' table.
 *
 * 
 *
...
abstract class BaseRoleaction extends BaseObject  implements Persistent
{
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = RoleactionPeer::ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = RoleactionPeer::NUM_HYDRATE_COLUMNS.
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(RoleactionPeer::ID) ) {
	...
	{
		if (isset($alreadyDumpedObjects['Roleaction'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Roleaction'][$this->getPrimaryKey()] = true;
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RoleactionPeer::ID)) $criteria->add(RoleactionPeer::ID, $this->id);
	...
	{
		$criteria = new Criteria(RoleactionPeer::DATABASE_NAME);
		$criteria->add(RoleactionPeer::ID, $this->id);
	...
	{
		return (string) $this->exportTo(RoleactionPeer::DEFAULT_STRING_FORMAT);
	}
