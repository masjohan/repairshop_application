 ...
 * @method     ActionQuery orderByActionCategoryId($order = Criteria::ASC) Order by the action_category_id column
 *
 ...
 * @method     ActionQuery groupByActionCategoryId() Group by the action_category_id column
 *
 ...
 *
 * @method     ActionQuery leftJoinActioncategory($relationAlias = null) Adds a LEFT JOIN clause to the query using the Actioncategory relation
 * @method     ActionQuery rightJoinActioncategory($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Actioncategory relation
 * @method     ActionQuery innerJoinActioncategory($relationAlias = null) Adds a INNER JOIN clause to the query using the Actioncategory relation
 *
 ...
 * @method     Action findOneByActionCategoryId(int $action_category_id) Return the first Action filtered by the action_category_id column
 *
 ...
 * @method     array findByActionCategoryId(int $action_category_id) Return Action objects filtered by the action_category_id column
 *
...
abstract class BaseActionQuery extends ModelCriteria
{
	...
	/**
	 * Filter the query on the action_category_id column
	 * 
	 * Example usage:
	 * <code>
	 * $query->filterByActionCategoryId(1234); // WHERE action_category_id = 1234
	 * $query->filterByActionCategoryId(array(12, 34)); // WHERE action_category_id IN (12, 34)
	 * $query->filterByActionCategoryId(array('min' => 12)); // WHERE action_category_id > 12
	 * </code>
	 *
	 * @see       filterByActioncategory()
	 *
	 * @param     mixed $actionCategoryId The value to use as filter.
	 *              Use scalar values for equality.
	 *              Use array values for in_array() equivalent.
	 *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 ...
	 * @return    ActionQuery The current query, for fluid interface
	 */
	public function filterByActionCategoryId($actionCategoryId = null, $comparison = null)
	{
		if (is_array($actionCategoryId)) {
			$useMinMax = false;
			if (isset($actionCategoryId['min'])) {
				$this->addUsingAlias(ActionPeer::ACTION_CATEGORY_ID, $actionCategoryId['min'], Criteria::GREATER_EQUAL);
				$useMinMax = true;
			}
			if (isset($actionCategoryId['max'])) {
				$this->addUsingAlias(ActionPeer::ACTION_CATEGORY_ID, $actionCategoryId['max'], Criteria::LESS_EQUAL);
				$useMinMax = true;
			}
			if ($useMinMax) {
				return $this;
		...
		return $this->addUsingAlias(ActionPeer::ACTION_CATEGORY_ID, $actionCategoryId, $comparison);
	}

	/**
	 * Filter the query by a related Actioncategory object
	 *
	 * @param     Actioncategory|PropelCollection $actioncategory The related object(s) to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    ActionQuery The current query, for fluid interface
	 */
	public function filterByActioncategory($actioncategory, $comparison = null)
	{
		if ($actioncategory instanceof Actioncategory) {
			return $this
				->addUsingAlias(ActionPeer::ACTION_CATEGORY_ID, $actioncategory->getId(), $comparison);
		} elseif ($actioncategory instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
				->addUsingAlias(ActionPeer::ACTION_CATEGORY_ID, $actioncategory->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
			throw new PropelException('filterByActioncategory() only accepts arguments of type Actioncategory or PropelCollection');
		}
	}

	/**
	 * Adds a JOIN clause to the query using the Actioncategory relation
	 * 
	 ...
	 * @return    ActionQuery The current query, for fluid interface
	 */
	public function joinActioncategory($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
	{
		$tableMap = $this->getTableMap();
		$relationMap = $tableMap->getRelation('Actioncategory');
		...
		if($relationAlias) {
			...
			$this->addJoinObject($join, 'Actioncategory');
		}
	...
	/**
	 * Use the Actioncategory relation Actioncategory object
	 *
	 * @see       useQuery()
	 * 
	 * @param     string $relationAlias optional alias for the relation,
	 ...
	 * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
	 *
	 * @return    ActioncategoryQuery A secondary query class using the current class as primary query
	 */
	public function useActioncategoryQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
	{
		return $this
			->joinActioncategory($relationAlias, $joinType)
			->useQuery($relationAlias ? $relationAlias : 'Actioncategory', 'ActioncategoryQuery');
	}
