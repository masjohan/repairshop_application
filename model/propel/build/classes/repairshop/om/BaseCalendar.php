...
abstract class BaseCalendar extends BaseObject  implements Persistent
{
	...
	public function setResourceId($v)
	{
		...
		if ($this->resource_id !== $v) {
			$this->resource_id = $v;
			$this->modifiedColumns[] = CalendarPeer::RESOURCE_ID;
		}

		if ($this->aCalendarresource !== null && $this->aCalendarresource->getId() !== $v) {
	...
	public function setSlotId($v)
	{
		...
		if ($this->slot_id !== $v) {
			$this->slot_id = $v;
			$this->modifiedColumns[] = CalendarPeer::SLOT_ID;
		}

		if ($this->aCalendarslot !== null && $this->aCalendarslot->getId() !== $v) {
	...
	public function setEventId($v)
	{
		...
		if ($this->event_id !== $v) {
			$this->event_id = $v;
			$this->modifiedColumns[] = CalendarPeer::EVENT_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->slot_id = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			...
			return $startcol + 4; // 4 = CalendarPeer::NUM_HYDRATE_COLUMNS.
	...
	public function ensureConsistency()
	{
		...
		if ($this->aCalendarresource !== null && $this->resource_id !== $this->aCalendarresource->getId()) {
			$this->aCalendarresource = null;
		}
		if ($this->aCalendarslot !== null && $this->slot_id !== $this->aCalendarslot->getId()) {
			$this->aCalendarslot = null;
		}
		if ($this->aCalendarevent !== null && $this->event_id !== $this->aCalendarevent->getId()) {
			$this->aCalendarevent = null;
		}
	} // ensureConsistency
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->aCalendarslot !== null) {
				...
				$this->setCalendarslot($this->aCalendarslot);
			}
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(CalendarPeer::ID) ) {
	...
	{
		if (isset($alreadyDumpedObjects['Calendar'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Calendar'][$this->getPrimaryKey()] = true;
		$keys = CalendarPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getResourceId(),
			$keys[2] => $this->getSlotId(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setSlotId($arr[$keys[2]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CalendarPeer::RESOURCE_ID)) $criteria->add(CalendarPeer::RESOURCE_ID, $this->resource_id);
		if ($this->isColumnModified(CalendarPeer::SLOT_ID)) $criteria->add(CalendarPeer::SLOT_ID, $this->slot_id);
		if ($this->isColumnModified(CalendarPeer::EVENT_ID)) $criteria->add(CalendarPeer::EVENT_ID, $this->event_id);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setSlotId($this->getSlotId());
	 ...
	 * @throws     PropelException
	 */
	public function setCalendarslot(Calendarslot $v = null)
	{
		if ($v === null) {
			$this->setSlotId(NULL);
		} else {
		...
		$this->aCalendarslot = $v;

		// Add binding for other direction of this n:n relationship.
		// If this object has already been added to the Calendarslot object, it will not be re-added.
		if ($v !== null) {
			$v->addCalendar($this);
		}
	...
	/**
	 * Get the associated Calendarslot object
	 *
	 * @param      PropelPDO Optional Connection object.
	 * @return     Calendarslot The associated Calendarslot object.
	 * @throws     PropelException
	 */
	public function getCalendarslot(PropelPDO $con = null)
	{
		if ($this->aCalendarslot === null && ($this->slot_id !== null)) {
			$this->aCalendarslot = CalendarslotQuery::create()->findPk($this->slot_id, $con);
			/* The following can be used additionally to
				guarantee the related object contains a reference
				to this object.  This level of coupling may, however, be
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aCalendarslot->addCalendars($this);
			 */
		}
		return $this->aCalendarslot;
	}
	...
	{
		if ($this->aCalendarevent === null && ($this->event_id !== null)) {
			$this->aCalendarevent = CalendareventQuery::create()->findPk($this->event_id, $con);
			/* The following can be used additionally to
				guarantee the related object contains a reference
				to this object.  This level of coupling may, however, be
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aCalendarevent->addCalendars($this);
			 */
		}
	...
	{
		return (string) $this->exportTo(CalendarPeer::DEFAULT_STRING_FORMAT);
	}
