...
abstract class BaseSession extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the device_key field.
	 * @var        string
	 */
	...
	/**
	 * Get the [device_key] column value.
	 * 
	 * @return     string
	 */
	...
	public function getUpdatedOn($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->updated_on === '0000-00-00 00:00:00') {
			...
			try {
				$dt = new DateTime($this->updated_on);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->updated_on, true), $x);
			}
		}
	...
	/**
	 * Set the value of [device_key] column.
	 * 
	 * @param      string $v new value
	 * @return     Session The current object (for fluent API support)
	 */
	public function setDeviceKey($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->device_key !== $v) {
			$this->device_key = $v;
			$this->modifiedColumns[] = SessionPeer::DEVICE_KEY;
		}
	...
	public function setUpdatedOn($v)
	{
		...
		if ($this->updated_on !== null || $dt !== null) {
			$currentDateAsString = ($this->updated_on !== null && $tmpDt = new DateTime($this->updated_on)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->updated_on = $newDateAsString;
				$this->modifiedColumns[] = SessionPeer::UPDATED_ON;
			}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SessionPeer::UPDATED_ON)) $criteria->add(SessionPeer::UPDATED_ON, $this->updated_on);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setDeviceKey($this->getDeviceKey());
