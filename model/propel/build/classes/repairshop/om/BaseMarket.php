...
/**
 * Base class that represents a row from the 'Market' table.
 *
 * 
 *
 * @package    propel.generator.repairshop.om
 */
abstract class BaseMarket extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'MarketPeer';
	 ...
	 * @return     Market The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = MarketPeer::ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 1; // 1 = MarketPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Market object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = MarketPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				MarketPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(MarketPeer::ID) ) {
					...
					$affectedRows = MarketPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = MarketPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = MarketPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
	{
		...
		$alreadyDumpedObjects['Market'][$this->getPrimaryKey()] = true;
		$keys = MarketPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(MarketPeer::ID)) $criteria->add(MarketPeer::ID, $this->id);
	...
	{
		$criteria = new Criteria(MarketPeer::DATABASE_NAME);
		$criteria->add(MarketPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Market (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Market Clone of current object.
	 * @throws     PropelException
	 */
	...
	{
		return (string) $this->exportTo(MarketPeer::DEFAULT_STRING_FORMAT);
	}
